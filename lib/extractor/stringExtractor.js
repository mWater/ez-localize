"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.findInTsx = exports.findInTs = exports.findInHbs = exports.findInCoffee = exports.findInJs = exports.findFromRootDirs = void 0;
// TODO: This file was created by bulk-decaffeinate.
// Sanity-check the conversion and remove this comment.
const fs_1 = __importDefault(require("fs"));
const glob_1 = __importDefault(require("glob"));
const path_1 = __importDefault(require("path"));
const coffeescript_1 = __importDefault(require("coffeescript"));
const handlebars_1 = __importDefault(require("handlebars"));
const acorn_1 = __importDefault(require("acorn"));
const acorn_walk_1 = __importDefault(require("acorn-walk"));
const typescript_1 = __importDefault(require("typescript"));
// rootDirs are the directories to find files in. node_modules is never entered. Can be files as well, in which case the file is used
// callback is called with list of strings
function findFromRootDirs(rootDirs, callback) {
    let strings = [];
    for (let rootDir of rootDirs) {
        var filenames;
        if (fs_1.default.lstatSync(rootDir).isDirectory()) {
            filenames = glob_1.default.sync("**/*.@(js|coffee|tsx|ts|hbs)", { cwd: rootDir });
        }
        else {
            filenames = ["."];
        }
        for (let filename of filenames) {
            // Skip node_modules
            var fullFilename;
            if (filename.match(/node_modules/)) {
                continue;
            }
            if (filename !== ".") {
                fullFilename = path_1.default.resolve(rootDir, filename);
            }
            else {
                fullFilename = path_1.default.resolve(rootDir);
            }
            console.log(fullFilename);
            const contents = fs_1.default.readFileSync(fullFilename, "utf-8");
            const ext = path_1.default.extname(fullFilename);
            switch (ext) {
                case ".coffee":
                    strings = strings.concat(exports.findInCoffee(contents));
                    break;
                case ".js":
                    strings = strings.concat(exports.findInJs(contents));
                    break;
                case ".hbs":
                    strings = strings.concat(exports.findInHbs(contents));
                    break;
                case ".ts":
                    strings = strings.concat(exports.findInTs(contents));
                    break;
                case ".tsx":
                    strings = strings.concat(exports.findInTsx(contents));
                    break;
            }
        }
    }
    return callback(strings);
}
exports.findFromRootDirs = findFromRootDirs;
function findInJs(js) {
    const items = [];
    acorn_walk_1.default.simple(acorn_1.default.parse(js), {
        CallExpression: function (node) {
            var _a, _b, _c, _d, _e, _f, _g;
            if (((_a = node.callee) === null || _a === void 0 ? void 0 : _a.name) === "T" && typeof ((_b = node.arguments[0]) === null || _b === void 0 ? void 0 : _b.value) === "string") {
                return items.push((_c = node.arguments[0]) === null || _c === void 0 ? void 0 : _c.value);
            }
            else if (((_e = (_d = node.callee) === null || _d === void 0 ? void 0 : _d.property) === null || _e === void 0 ? void 0 : _e.name) === "T" && typeof ((_f = node.arguments[0]) === null || _f === void 0 ? void 0 : _f.value) === "string") {
                return items.push((_g = node.arguments[0]) === null || _g === void 0 ? void 0 : _g.value);
            }
        }.bind(this)
    });
    return items;
}
exports.findInJs = findInJs;
function findInCoffee(cs) {
    // Compile coffeescript
    const js = coffeescript_1.default.compile(cs);
    return exports.findInJs(js);
}
exports.findInCoffee = findInCoffee;
function findInHbsProgramNode(node) {
    let items = [];
    for (let stat of node.statements) {
        if (stat.type === "mustache" && stat.id.string === "T") {
            items.push(stat.params[0].string);
        }
        if (stat.type === "block") {
            if (stat.program) {
                items = items.concat(findInHbsProgramNode(stat.program));
            }
            if (stat.inverse) {
                items = items.concat(findInHbsProgramNode(stat.inverse));
            }
        }
    }
    return items;
}
function findInHbs(hbs) {
    const items = [];
    const tree = handlebars_1.default.parse(hbs);
    return findInHbsProgramNode(tree);
}
exports.findInHbs = findInHbs;
function findInTs(ts) {
    const js = typescript_1.default.transpileModule(ts, {
        compilerOptions: { module: typescript_1.default.ModuleKind.CommonJS }
    });
    return exports.findInJs(js.outputText);
}
exports.findInTs = findInTs;
function findInTsx(tsx) {
    const js = typescript_1.default.transpileModule(tsx, {
        compilerOptions: {
            module: typescript_1.default.ModuleKind.CommonJS,
            jsx: typescript_1.default.JsxEmit.ReactJSX
        }
    });
    return exports.findInJs(js.outputText);
}
exports.findInTsx = findInTsx;
